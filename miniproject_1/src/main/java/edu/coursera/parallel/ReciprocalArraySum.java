...
public final class ReciprocalArraySum {
	...
	private static class ReciprocalArraySumTask extends RecursiveAction {
		static final int SEQUENTIAL_LIMIT = 10000;
		/**
		...
		ReciprocalArraySumTask(final int setStartIndexInclusive, final int setEndIndexExclusive,
				final double[] setInput, final int nTasks) {
		...
		protected void compute() {
			if (endIndexExclusive - startIndexInclusive <= getChunkSize(nTasks, input.length)) {
				...
				*/
				...
				for (int chunk = 0; chunk < nTasks; chunk++) {
					int startInclusive = getChunkStartInclusive(chunk, nTasks, input.length);
					int endExclusive = getChunkEndExclusive(chunk, nTasks, input.length);
					
					tasks[chunk] = new ReciprocalArraySumTask(startInclusive, endExclusive, input, nTasks);
				}

				for (int chunk = 0; chunk < nTasks - 1; chunk++) {
					tasks[chunk].fork();
				}
				
				tasks[nTasks - 1].compute();

				for (int chunk = 0; chunk < nTasks - 1; chunk++) {
					tasks[chunk].join();
				}
				for (int chunk = 0; chunk < nTasks; chunk++) {
					value += tasks[chunk].getValue();
				}
			}
		}
	}
	...
	protected static double parArraySum(final double[] input) {
		...
		ReciprocalArraySumTask sumTask = new ReciprocalArraySumTask(0, input.length, input, 2);
	...
	protected static double parManyTaskArraySum(final double[] input, final int numTasks) {
		...
		SumWrapperTask sumTask = new SumWrapperTask(numTasks, input);
